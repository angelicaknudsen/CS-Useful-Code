...
# Jelly's input: YeAhHhHHHHHHHHHHHH BRUTE FORCING
for c in itertools.product(alphabet, repeat=3):
    ...
    # Jelly's input: lmao I'll definitely be deleting this in the future
    #time.sleep(0.001)
...
'''Jelly's additional notes:
With a little additional tweaking, you can make tons of different
types of brute-forcing code. For a challenge specifically, I just took a text file,
put each word as an element in a giant list (used .split())
and used simple indexing to change cases and add numbers (made a giant loop lollllll)
to test whether it would extract the file
'''
